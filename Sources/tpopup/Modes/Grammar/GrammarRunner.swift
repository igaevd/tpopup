...
/// Drives the grammar-correction flow.
///
...
///   3. Send the clipboard text to OpenAI with the user's grammar prompt.
...
@MainActor
final class GrammarRunner: NSObject {
    private let statusWindow = GrammarStatusWindow()
    ...
    func run() {
        ...
        let settings = SettingsStore.shared.grammar
        if let missing = settings.missingFields {
            let info: String
            if missing.count >= 3 {
                info = "Open the app to fill in your grammar correction settings."
            } else {
                info = "Open the app and set the \(joined(missing)) in the Grammar Correction tab."
            }
            ErrorPresenter.showAndQuit("Grammar correction settings are incomplete.", info: info)
        }
        ...
        guard let source = Clipboard.readText() else {
            ErrorPresenter.showAndQuit(
                "Nothing to correct.",
                info: "Copy some text first, then trigger grammar correction again."
            )
        }
    ...
    private func finishFailure(_ message: String) {
        statusWindow.hide()
        ErrorPresenter.showAndQuit("Grammar correction failed.", info: message)
    }
