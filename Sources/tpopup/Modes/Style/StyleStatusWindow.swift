...
/// Borderless, transparent floating window that hosts the style-correction status pill.
///
...
/// editor's text selection is preserved, so the replacement at the end actually lands on
/// the selection instead of some unrelated control.
@MainActor
final class StyleStatusWindow {
    private var window: NSWindow?

    func show(onScreen screen: NSScreen) {
        let hosting = NSHostingView(rootView: StyleStatusView())
        let size = StyleStatusView.size
